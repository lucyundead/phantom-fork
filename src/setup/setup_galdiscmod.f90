...
!
! set code units
 call set_units(dist=1000.*pc,mass=1.d010*solarm,G=1.)
...
!
 ...
 if (maxvxyzu >= 4) then
    ...
    thermal = 12124.1  !corresponds to cs=10km/s for HI 
...
!
  ...
  psep = 1.1292*hfact*(massoftype(1)/rhozero)**(1./3.)
  ...
  call set_unifdis('closepacked',id,master,xmin,xmax,ymin,ymax,zmin,zmax,psep,&
                hfact,npart,xyzh,rcylmin=rcylin,rcylmax=rcyl,nptot=npart_total)
...
! rhofuncr = 71.76*exp(-r/4.8)*(solarm/pc**2)/(umass/(udist**2))

 if (r <= 5.0) then
     rhofuncr = 25.3*(solarm/pc**2)/(umass/(udist**2))
 else 
     rhofuncr = 71.76*exp(-r/4.8)*(solarm/pc**2)/(umass/(udist**2))
 endif
